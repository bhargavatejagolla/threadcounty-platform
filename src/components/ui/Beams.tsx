...
const CanvasWrapper = ({ children }: { children: React.ReactNode }) => (
  <Canvas dpr={[1, 2]} frameloop="always" className="beams-container" camera={{ position: [0, 0, 20], fov: 30 }}>
    {children}
  </Canvas>
);
  ...
  return (
    <CanvasWrapper>
      <group rotation={[0, 0, degToRad(rotation)]}>
        ...
        <DirLight color={lightColor} position={[0, 3, 10] as [number, number, number]} />
      </group>
...
const MergedPlanes = forwardRef((props: any, ref: any) => {
  const { material, width, count, height } = props;
  ...
  useFrame((_, delta) => {
    if (mesh.current && mesh.current.material && mesh.current.material.uniforms.time) {
